...
char random_convert( char base )
{
    ...
    default :
            result = 'n';
            fprintf( stderr, "%s base(%c, %d, 0x%x) is not normal\n", ERR_004, base, base, base );
            break;
    }
    return result;
}
    ...
    while( ( l = kseq_read( seq ) ) >= 0 )
    {
        ...
        for( i = start; i <= (seq->seq.l - end_offset); i = i+sampling )
        {
            ...
            for( cn = 0; cn < 1; cn++ )  // copy number
            {
                ...
                //fprintf( stderr, ">%s|%09d(%d) (L%09d-%09d)\n", seq->name.s, offset, cn+1, offset, offset+length-1 );
                read_length = length;
                ...
                {   
                    ...
                    else if( r < ( (error_sub + error_ins)*10000) ) // insertion
                    {
                        ...
                        //{
                            ...
                            memmove( &seqbuf[j+1], &seqbuf[j], read_length-j );
                    ...
                    {//    else  // delete
                            ...
                            //fprintf( stderr, "%s\n", &seqbuf[j] );
                            memmove( &seqbuf[j], &seqbuf[j+1], read_length-j-1 );
