...
void analyzer_long(int argc, char *argv[])  
{  
...
//    FILE *output dd= NULL;
    ...
    fprintf( stderr, "==================================================\n" );
    fprintf( stderr, "Analyzer for long reads\n" );
    ...
    prob_init_window( 120, length * 1.2 );
//    output = fopen( "sort.gma", "w+" );
    ...
    while( fgets( input, BUF_SIZE-1, stdin ) )
    {
        ...
        else // new chromosome
        {
            ...
            prob_long_clean_windows( NULL, prev_chr, prev_pos );  // NULL means stdout
            //prob_destroy_window();
            prob_init_window( 120, length * 1.2 );
        ...
        if( is_same_chr( org_chr, fnd_chr ) == true )      
        {
            fprintf( stderr, "%s org_pos(%d) <= fnd_pos1(%d) && fnd_pos2(%d) < (org_pos + length * 1.2 - 1)%d\n", 
                             INFO_027, org_pos, fnd_pos1, fnd_pos2, (int)(org_pos + length * 1.2)  );
            // check position availability

            if( ( (org_pos <= fnd_pos1) && ( (fnd_pos2) < (org_pos + length * 1.2 ) ) ) ||
                ( fnd_pos1 == 0 && fnd_pos2 == 99 ) )
            {
                if( is_new_pos == true )
                {
//                    fprintf( stderr, "%s new pos\n", INFO_004 );
                    prob_long_fill_window( NULL,
                                 distance,
    ...
    prob_long_clean_windows( NULL, org_chr, org_pos ); 
    fprintf( stderr, "%s destroy window\n", INFO_032 );
    //prob_destroy_window(); don't need for long reads sequence
}
