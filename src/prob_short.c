...
/*
static void circulate_window_list( prob_elem * hd, int read, char *chr, int fnd_pos1, int fnd_pos2, int length )
{
    int i = 0, j = 0; 
    int qual = 0;
    int cvg = 0;
    ...
    for( i = fnd_pos1-1; i < fnd_pos2-1; i++ )
    {
        ...
        for( j = 0; j < pw.filled ; j++ )
        {
            ...
            fprintf( stderr, "%s elem(%p) id(%d) offset:%d(%c) (%d/%d), qual[%d]:%d, p_err:%.4f, acc:%.4f cvg(%d)\n", INFO_015, elem, elem->id, elem->pos, elem->base, offset, pw.filled, read, elem->maq[read][offset] , p_err, p_acc, cvg );
            elem = elem->next;
        }
...
static void compute_gms( prob_elem * head, char *chr, int fnd_pos1, int fnd_pos2, int length )
{
    circulate_window_list( head, READ1, chr, fnd_pos1, fnd_pos2 );
}


void prob_short_fill_elem( prob_elem * pb, 
                     ...
                     char base, int read, int qual, int flag, int length  )
{
    ...
    else if( ( flag & 0x10 ) || ( flag == 0 ) || ( flag == 0x4 )  )  // if flag == 0, it's single end read or cannot  know which read if comes from if paired-end read
    {
        ...
        {
            fprintf( stderr, "%s prob_short_check_fill_elem: flag: not first, not second\n", ERR_005 );
        }
    }
    else 
    {
...
void prob_short_check_fill_elem( prob_elem * pb, 
                           ...
                           char base, int read, int qual, int flag, int length  )
{
    ...
    if( pw.head->pos == 0 )  
    {
        prob_short_fill_elem( pw.head, org_pos, fnd_pos1, fnd_pos2, base, read, qual, flag, length );
        pw.filled++;
        pw.tail = pw.head;
        //fprintf( stderr, "%s case 1: pw.filled:%d (%d - %d)\n", INFO_018, pw.filled, pw.head->pos, pw.tail->pos );
    }
    else// if( pw.head->pos != 0 )
    {
        if( pw.tail->pos == org_pos) 
        {
            prob_short_fill_elem( pw.tail, org_pos, fnd_pos1, fnd_pos2, base, read, qual, flag, length );
//            fprintf( stderr, "%s case 2: pw.filled:%d (%d - %d)\n", INFO_019, pw.filled, pw.head->pos, pw.tail->pos );
        }
        else if( pw.tail->pos < org_pos )
        {    
            fprintf( stderr, "%s pw.tail->next: %p, pw.head->pos: %d, pw.filled:%d, pw.size:%d\n",
                              INFO_020, pw.tail->next, pw.head->pos, pw.filled, pw.size );

            if( pw.filled < pw.size) 
            {
                prob_short_fill_elem( pw.tail->next, org_pos, fnd_pos1, fnd_pos2, base, read, qual, flag, length );
                pw.tail = pw.tail->next;
                pw.filled++;
//                fprintf( stderr, "%s case 3: pw.filled:%d (%d - %d), pw.tail->id: %d\n", INFO_022, pw.filled, pw.head->pos, pw.tail->pos, pw.tail->id );
            }
...
int prob_short_fill_window( FILE * output, int distance, char *chr, 
                      int org_pos, int fnd_pos1, int fnd_pos2, 
                      char base, int flag, int qual, int length )
{
    ...
    {
        prob_short_check_fill_elem( pw.head, chr, org_pos, fnd_pos1, fnd_pos2, base, READ1, qual, flag, length );
    }
    ...
    {
        prob_short_check_fill_elem( pw.head, chr, org_pos, fnd_pos1, fnd_pos2, base, READ2, qual, flag, length );
    }
    ...
    else if( ( flag == 0 ) || ( flag & 0x10 ) || ( flag & 0x4) )// single-end read, unmapped
    {
        ...
        prob_short_check_fill_elem( pw.head, chr, org_pos, fnd_pos1, fnd_pos2, base, -1, qual, flag, length );
    }
    else if( ( fnd_pos1 - pw.head->pos ) == 1 )
    {
        fprintf( stderr, "%s here\n", ERR_007 );
        prob_short_check_fill_elem( pw.head, chr, org_pos, fnd_pos1, fnd_pos2, base, -1, qual, flag, length );  //flag
    }
    else
    {
        fprintf( stderr, "%s here\n", ERR_008 );
...
int prob_short_move_window( FILE * output, int distance, char *org_chr, 
                      int org_pos, int fnd_pos1, int fnd_pos2, 
                      char base, int flag, int qual, int length )
{
    compute_gms( pw.head, org_chr, fnd_pos1, fnd_pos2, length );
...
/*
 *
static void cleanup_window_list( prob_elem * hd, int read, char *chr, int pos1, int pos2 )
{
    int i = 0, j = 0;
    int qual = 0;
    int cvg = 0;
