...
static int parse_args( int argc, char *argv[], 
                char *tech, int *length, char *qual, 
                float *error_sub_rate, float *error_ins_rate, float *error_del_rate, 
                int *range, char *fasta, char *index_path,
    ...
    while( ( c = getopt( argc, argv, "l:q:s:i:d:o:t:f:b:x:m:p:" )) >= 0 ) 
    {
        ...
        case 'p': 
                  ...
                  { 
                      ...
                      //fprintf( stderr, "[INFO] hadoop bit is set.\n" );
                  }
                  break;
...
static int run_bwa( char *tech, char *fasta, char *fastq_se, char *fastq_pe, int hadoop, char *bwa_path, int length ) //, int length  )
{
    pid_t pid;
    int fd = 0;
    int error = 0;
    ...
    if( length > BOUNDARY_LEN )
    {
        ...
        {   // child process   
            ...
            npro = sysconf(_SC_NPROCESSORS_ONLN);
            if ( npro <= 0 )
            {    npro = 1; 
            }
            fprintf( stderr, "The number of CPUs : %d\n", npro );
            sprintf( smp, "-t%d", npro );
            fprintf( stderr, "We do support multi-threading to speed up. %s is enabled.\n", smp );

            if( hadoop == TRUE )
            {
                 
                // fprintf( stderr, "BWA running on hadoop......\n");
                if( strcmp( tech, "pacbio" ) == 0 )
                { 
                    fprintf( stderr, "[command] bwa bwasw -b5 -q2 -r1 -z10 %s %s %s > aln_se.sam\n", smp, fasta, fastq_se );
                    error = execl( bwa_path, "bwa", "bwasw", "-b5", "-q2", "-r1", "-z10", smp,  fasta, fastq_se, (char *) 0);     
                }
                else
                {
            ...
            {
                // fprintf( stderr, "BWA running locally......\n");
                if( strcmp( tech, "pacbio" ) == 0 )
                { 
                    fprintf( stderr, "[command] bwa bwasw -b5 -q2 -r1 -z10 %s %s %s > aln_se.sam\n", smp, fasta, fastq_se );
                    //error = execl("../../../../../bin/bwa", "bwa", "bwasw", "-b5", "-q2", "-r1", "-z10", fasta, fastq_se, (char *) 0);  

                    error = execl("/bluearc/data/schatz/hlee/program/bwa", "bwa", "bwasw", "-b5", "-q2", "-r1", "-z10", smp, fasta, fastq_se, (char *) 0);     
                }
                else
                {
                    //error = execl("../../../../../bin/bwa", "bwa", "bwasw", fasta, fastq_se, (char *) 0);     
                    error = execl("/local2/work/hlee/schatzlab-public/gma/bin/bwa", "bwa", "bwasw", fasta, fastq_se, (char *) 0);     
                }
            }

            if( error < 0 )
            {
                fprintf( stderr, "BWA bwasw - FAIL(error:%d)!\n\n", error );
            }
            else
            {
    ...
    else
    {
        ...
        {   // child process   
            ...
            else
            {
                ...
                error = execl("/local2/work/hlee/schatzlab-public/gma/bin/bwa", "bwa", "aln", fasta, fastq_se, (char *) 0);
            }
        ...
        {   // child process   
            ...
            else
            {
                ...
                error = execl("/local2/work/hlee/schatzlab-public/gma/bin/bwa", "bwa", "samse", fasta, "aln_se.sai", fastq_se, (char *) 0);     
            }
    ...
    { 
        ...
        else
        {
            ...
            error = execl("/local2/work/hlee/schatzlab-public/gma/bin/bwa", "bwa", "aln", fasta, fastq_pe, (char *) 0);
        }
#endif
            if( error < 0 )
            {
        ...
        {   // child process   
            ...
            else
            {
                ...
                error = execl("/local2/work/hlee/schatzlab-public/gma/bin/bwa", "bwa", "sampe", fasta, "aln_se.sai", "aln_pe.sai", fastq_se, fastq_pe, (char *) 0);
            }
#endif
            if( error < 0 )
            {
                                      ...
                                      const char *samtools_path )
{
    ...
    {   // child process 
        ...
        else
        {
            ...
            error = execl("/local2/work/hlee/schatzlab-public/gma/bin/samtools", "samtools", "faidx", fasta, (char *) 0);
        }
    ...
    {   // child process   
        ...
        else
        {
            ...
            error = execl("/local2/work/hlee/schatzlab-public/gma/bin/samtools", "samtools", "import", fai, aln_sam, aln_bam, (char *) 0); 
        } 
    ...
    {   // child process   
        ...
        else
        {
            ...
            error = execl("/local2/work/hlee/schatzlab-public/gma/bin/samtools", "samtools", "sort", aln_bam, aln_sort, (char *) 0);
        }
#endif
        if( error < 0 )
        {
...
{  
    ...
    parse_args( argc, argv, 
                tech, &length, &qual, &error_sub_rate, &error_ins_rate, &error_del_rate, 
                &ob_range, fasta, index_path, &hadoop, prog_path );
    ...
    //fprintf( stderr, "is hadoop : %d (0:local, 1:hadoop)\n", hadoop );
    //fprintf( stderr, "program path (if GMA is running in hadoop): %s\n", prog_path );
...
//    run_exc_ls( "." );
    ...
    if( ( length > BOUNDARY_LEN ) && ( ib_range > 0 ) )
    {
    ...
    {
        run_bwa( tech, fasta, fastq_se, fastq_pe, hadoop, bwa_path, length );
    }
    else
    {
        run_bwa( tech, fasta, fastq_se, NULL, hadoop, bwa_path, length );
    }
    ...
    if( ib_range > 0 ) 
    {
        parse_bam( tech, fasta, length, start_base_pos, aln_pe_sort_bam );
    }
    else
    {
        if( length > BOUNDARY_LEN )
        {
            parse_bam( tech, fasta, length, start_base_pos, aln_se_bam );
        }
        else
        {
            parse_bam( tech, fasta, length, start_base_pos, aln_se_sort_bam );
        }
    }
