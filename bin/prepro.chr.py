            ...
            if line[0] == '>': # new chromosome
                ...
                if chr.isdigit() == True or chr == "X" or chr == "Y" :
                    ...
                    if ( "MT" in line ) or ( "mito" in line ) :
                       ...
                       #chr = filename.split('/')[len(filename.split('/'))-1]
                       chr = filename.split('/')[-1].split('.')[0]

                fa_out.close()
                #fa_out = open( chr + '.ppd', 'w' )
                #print str(chr+'.ppd'), "is being processed"
                fa_out = open( filename.split('/')[-1]  + '.ppd', 'w' )
                print str( filename.split('/')[-1] +'.ppd'), "is being processed"
